    ...
    const patchNote = async () => {
        ...
        try {
          await fetch(`http://localhost:4000/patchNote/${note._id}`, {
            ...
            body: JSON.stringify({
              title: note.title || undefined, // Use undefined to skip updating if not provided
          ...
          .then(async (response) => {
            if (!response.ok) {
              ...
              setStatus(`Error trying to update note: ${response.status}`);
            } else {
              await response.json().then((data) => {
          ...
          setStatus("Error trying to update note2");
          console.log("Fetch function failed:", error);
        }
      };
